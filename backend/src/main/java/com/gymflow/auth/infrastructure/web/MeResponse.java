...
public record MeResponse(UUID userId, String email, String role, UUID gymId) {

    public static MeResponse from(AuthenticatedPrincipal principal) {
        return new MeResponse(principal.userId(), principal.email(), principal.role().name(), principal.gymId());
    }
}
